...
def train_manifold():
    ...
    for gen in range(1, n_generations + 1):
        ...
        for i in range(10): # Epochs
            ...
            # Prediction Loss (Stability/Consistency)
            # Use state[t+1] as target for prediction at t
            # prediction: (T, ...)
            # flat_state: (T, ...)
            # We align them: prediction[t] -> flat_state[t+1]
        ...
        print(f"Gen {gen} | Loss: {loss.item():.4f} | Pred Error: {loss_pred.item():.4f} | Energy: {energy.mean().item():.4f}")
